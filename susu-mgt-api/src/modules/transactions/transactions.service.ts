...
export class TransactionsService {
  ...
  async createWithdrawalRequest(params: {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const pendingTotal = pendingAgg._sum.amount ?? new Prisma.Decimal(0);
      if (wallet.balance.lessThan(params.amount.plus(pendingTotal))) {
        const available = wallet.balance.minus(pendingTotal);
        throw new BadRequestException(
          `You don't have enough available funds for this withdrawal. Your available balance is GHS${available.toString()}.`,
        );
      }
      ...
      if (params.method === 'CASH') {
        try {
          ...
          if (workerNotifications.length > 0) {
            await tx.notification.createMany({ data: workerNotifications });
          }
        } catch (err) {
          this.logger.error(
  ...
  async approveWithdrawalRequest(params: {
    ...
    return this.prisma.$transaction(
      async (trx) => {
        ...
        // Ensure enough balance considering other pending withdrawals
        const otherPendingAgg = await trx.transaction.aggregate({
          where: {
            userId: tx.userId,
            type: TransactionType.WITHDRAWAL,
            status: TransactionStatus.PENDING,
        ...
        const otherPending =
          otherPendingAgg._sum.amount ?? new Prisma.Decimal(0);
        if (wallet.balance.lessThan(tx.amount.plus(otherPending))) {
          const available = wallet.balance.minus(otherPending);
          throw new BadRequestException(
            `You don't have enough available funds for this withdrawal. Your available balance is ${available.toString()}.`,
          );
        }
