    ...
    <header className="fixed top-0 z-50 w-full border-b border-white/10 bg-[#0a0a0a]/90 backdrop-blur-md">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:px-8">
        ...
        <div className="flex items-center gap-2">
          <a href="#home" className="flex items-center gap-2 transition-transform active:scale-95">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {isOpen ? <X size={28} className="text-green-400" /> : <Menu size={28} />}
          </button>
        </div>
      </nav>
      ...
      <div 
        className={`absolute left-0 w-full overflow-hidden bg-[#0f0f0f] transition-all duration-500 ease-in-out lg:hidden ${
          isOpen ? "max-h-[450px] opacity-100 border-b border-white/10" : "max-h-0 opacity-0"
        }`}
      >
