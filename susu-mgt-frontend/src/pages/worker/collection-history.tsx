...
export function CollectionHistoryPage() {
  ...
  const fetchCollections = async () => {
    try {
      setIsLoading(true)
      const res = await workersService.getWorkerCollections({ page, limit })
    ...
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-zinc-900 dark:text-zinc-100">Collection History</h1>
            <p className="text-zinc-500 dark:text-zinc-400 mt-1">Review all your processed customer deposits.</p>
          </div>
        </div>
      </div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-zinc-500 dark:text-zinc-400">Customer: {tx.userId.slice(0, 8)}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
