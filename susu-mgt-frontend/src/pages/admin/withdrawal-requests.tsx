...
export function WithdrawalRequestsPage() {
  ...
  const fetchRequests = async () => {
    try {
      setIsLoading(true)
      const res = await adminService.getAllWithdrawals({ page, limit })
  ...
  const handleApprove = async (id: string) => {
    try {
      setIsProcessing(id)
      await transactionsService.approveWithdrawal(id, { remarks: 'Approved by admin' })
      await fetchRequests()
    } catch (err) {
  ...
  const handleReject = async (id: string) => {
    try {
      setIsProcessing(id)
      await transactionsService.rejectWithdrawal(id, { remarks: 'Rejected by admin' })
      await fetchRequests()
    } catch (err) {
  ...
  const handleConfirmPayment = async (id: string) => {
    if (!confirm('Are you sure you want to confirm that payment has been made? This will deduct the amount from the user\'s wallet.')) return
    try {
      setIsProcessing(id)
      await transactionsService.confirmWithdrawalPayment(id, { remarks: 'Payment confirmed by admin' })
      await fetchRequests()
    } catch (err) {
    ...
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-zinc-900 dark:text-zinc-100">Withdrawal Requests</h1>
            <p className="text-zinc-500 dark:text-zinc-400 mt-1">Manage and process customer withdrawal requests.</p>
          </div>
        </div>
      </div>
                ...
                {isLoading ? (
                    ...
                    <tr key={req.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-bold text-zinc-900 dark:text-zinc-100">{req.id.slice(0, 8).toUpperCase()}</p>
                            <p className="text-xs text-zinc-500 dark:text-zinc-400">User ID: {req.userId.slice(0, 8)}...</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <Badge 
                          ...
                          className={cn(req.status === 'APPROVED' && "bg-blue-50 text-blue-700 border-blue-200")}
                        >
                          {req.status}
                        </Badge>
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {req.status === 'PENDING' && (
                            <>
                              ...
                              >
                                {isProcessing === req.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <XCircle className="h-4 w-4" />}
                              </Button>
                            </>
                          )}
                          {req.status === 'APPROVED' && (
                            ...
                            >
                              {isProcessing === req.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <CreditCard className="h-4 w-4" />}
                            </Button>
                          )}
                        </div>
                      </td>
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-24 text-center">
                      ...
                      <p className="text-zinc-500 dark:text-zinc-400">No withdrawal requests found.</p>
                    </td>
                  </tr>
                )}
              </tbody>
