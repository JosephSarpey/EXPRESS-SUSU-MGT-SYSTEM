          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-zinc-50 dark:bg-zinc-900 border-b dark:border-zinc-800">
              <tr>
                ...
                <th className="px-3 py-2 font-bold text-zinc-500">Value / Change</th>
              </tr>
            </thead>
            <tbody className="divide-y dark:divide-zinc-800">
              {allKeys.map(key => {
                ...
                return (
                  <tr key={key} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/50 transition-colors">
                    <td className="px-3 py-2 font-medium text-zinc-700 dark:text-zinc-300">{key}</td>
                    <td className="px-3 py-2 text-blue-600 dark:text-blue-400 font-medium">
                      {newVal === null || newVal === undefined ? <span className="text-zinc-300 italic">none</span> : String(newVal)}
                    </td>
                  </tr>
                )
              })}
    ...
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-zinc-900 dark:text-zinc-100">Audit Logs</h1>
            <p className="text-zinc-500 dark:text-zinc-400 mt-1">System activity and administrative actions.</p>
          </div>
        </div>
      </div>
                    ...
                    <tr key={log.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-bold text-zinc-900 dark:text-zinc-100">{formatAction(log.action)}</p>
                            <p className="text-xs text-zinc-500 dark:text-zinc-400">{log.details || `ID: ${log.id.slice(0, 8)}`}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <Card className="w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <CardHeader className="flex flex-row items-center justify-between border-b dark:border-zinc-800 p-4 sticky top-0 bg-white dark:bg-zinc-950 z-10">
              ...
              <Button variant="ghost" size="icon" onClick={() => setSelectedLog(null)} className="rounded-full h-8 w-8">
            ...
            <CardContent className="p-4 space-y-4">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Badge variant="secondary" className="font-bold">{formatAction(selectedLog.action)}</Badge>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm font-semibold text-zinc-500">Resource (Entity Type)</p>
                  <p className="font-medium">{selectedLog.entityType || 'SYSTEM'}</p>
                </div>
                <div>
                  <p className="text-sm font-semibold text-zinc-500">Target ID</p>
                  <p className="font-medium break-all">{selectedLog.targetId || 'N/A'}</p>
                </div>
              </div>
              <div>
                <p className="text-sm font-semibold text-zinc-500">User</p>
                <p className="font-medium">{selectedLog.user?.fullName || selectedLog.actorId || 'System'}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm font-semibold text-zinc-500">IP Address</p>
                  <p className="font-medium">{selectedLog.ipAddress || 'Unknown'}</p>
                </div>
                <div>
                  <p className="text-sm font-semibold text-zinc-500">Timestamp</p>
                  <p className="font-medium">{selectedLog.createdAt ? format(new Date(selectedLog.createdAt), 'PPpp') : 'N/A'}</p>
                </div>
              </div>
              <div>
                <p className="text-sm font-semibold text-zinc-500">User Agent</p>
                <p className="font-medium text-xs break-all bg-zinc-50 dark:bg-zinc-900 p-2 rounded">{selectedLog.userAgent || 'Unknown'}</p>
              </div>
              ...
              {!selectedLog.oldValues && !selectedLog.newValues && (
                <div className="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-dashed dark:border-zinc-800 text-center">
                  <p className="text-xs text-zinc-500 italic">No detailed property changes recorded for this action.</p>
                </div>
              )}
